// Deno-Port von src/services/ai/overcorrection-guard.ts (1:1, siehe
// prompt-types.ts für den Hintergrund der Duplizierung).
//
// Hinweis: `overcorrectionPolicy` wird aktuell nirgendwo im System befüllt
// (kein DB-Feld, kein Aufrufer setzt es) -- auch im Client-Pfad
// (prompt-execution.service.ts) ist dieser Guard dadurch faktisch ein
// No-op. Er wird hier trotzdem 1:1 mitgeführt, damit process-session exakt
// dasselbe Verhalten zeigt wie der Client-Pfad, inklusive eines künftigen
// Wirksamwerdens, sobald eine Policy-Quelle existiert.

import type { OvercorrectionGuardPolicy } from './prompt-types.ts';
